		...
		if money >= amount then
			...
			thread(when, signal_received, function()
				...
				if name and amount > 0 and amount >= buyout_price then
                    ...
                    -- Hook para Accounting (Fix Robustez)
                    local accounting = aux.tabs and aux.tabs.trading and aux.tabs.trading.accounting
                    if not accounting and _G.aux and _G.aux.tabs and _G.aux.tabs.trading then
                        accounting = _G.aux.tabs.trading.accounting
                    end
                    
                    if accounting and accounting.record_purchase then
                        local link = GetAuctionItemLink(type, index)
                        if link then
                             accounting.record_purchase(link, amount, count, "Auction House")
                        end
                    end
				end
...
function handle.LOAD2()
    local account_data = aux.account_data or {}
	frame:SetScale(account_data.scale or 1)
end
